...
### Conversión de coordenadas (minimapa CIRCULAR)

Todo pasa por **`mmPlace(dot, worldX, worldY)`**: proyecta px de mundo al minimapa y, si el punto cae fuera del círculo, lo retiene pegado al borde (estilo radar de MMO — un enemigo en la esquina del mapa sigue visible en el borde).

```typescript
// mmScale = (MM_RADIUS * 2) / max(mapWidthPx, mapHeightPx)
// mmOffX/mmOffY centran el mapa proyectado en el círculo (mmCX, mmCY)
// mapWidthPx = currentMap.width (tiles) * GameScene.TILE_SIZE (48)
dx = mmOffX + worldX * mmScale - mmCX   // luego se limita a radio MM_RADIUS - dot.radius - 2
```

### Posición y estilo

Constantes `MM_*` al inicio de `mobile-hud.scene.ts`:
- `MM_TOP = 10`, `MM_MARGIN = 10` — pegado a la esquina superior derecha.
- `MM_RADIUS = 65` — radio del círculo en px de pantalla.
...
3. **`createMinimap()`**: dibujarlo una sola vez, igual que los portales:
   ...
   for (const c of data.chests) {
     const dot = this.add.circle(0, 0, MM_DOT_CHEST, MM_COLOR_CHEST, 0.9);
     this.mmPlace(dot, c.x, c.y);
   }
   ```
...
4. Para formas distintas a círculo: `this.add.rectangle(...)` o `this.add.star(...)` funcionan en el pool, pero `mmPlace()` está tipado para `Arc` (usa `dot.radius` para el clamp al borde) — generalizar la firma o pasar el radio aparte.

## Modificaciones frecuentes

| Quiero… | Dónde |
|---------|-------|
| Mover/redimensionar el minimapa | `MM_TOP`, `MM_MARGIN`, `MM_RADIUS` |
| Cambiar colores/tamaños de puntos | `MM_COLOR_*`, `MM_DOT_*` |
| Cambiar fondo/borde | `createMinimap()` — círculo `bg` (fill + stroke) |
