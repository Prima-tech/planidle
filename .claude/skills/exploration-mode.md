...
| `src/app/components/sprint-button/*` · `src/app/components/fly-button/*` | Botones de habilidad del runner (abajo-izda): **impulso** (sprint, ×2 con pico) y **vuelo/boost** (10 s, ×2 vel. + invuln + altura manual). Leen los getters de cooldown de `PlayerBridgeService` |
| `src/app/components/map-teleport-hint/*` | Icono de teletransporte (arriba-dcha) al cruzar el cartel de un mapa comprado → entrar a él (`requestEnterMap` → `enterMap`) |
...
- **Salir por el mapa mundial (viajar a Asgard u otro mapa desde el panel):** `WorldMapPanelComponent.teleport()`. En runMode NO usa `restartGameScene` (rebotaría: `currentMapId` está obsoleto y `activity` sigue 'exploring' → `GameScene.create` reenvía al runner). En su lugar: `playerBridge.requestEnterMap(pinId)` → `enterMapRequest$` → `WorldRunScene.enterMap` (fade + `activity.set('idle')` + `setCurrentMap` + `scene.start('GameScene')`) y `requestCloseMenus()` para cerrar el panel. NO hace `goHomeReset`/cura (es viaje, no "fin de expedición").
...
| `hearts` | 20 | SIN él no aparecen corazones (gating de spawn en `updateHearts`, mismo patrón que `enemies`) |
...
| `boost` | 350 | botón de boost: 10 s ×5 vel. + invuln + arrasa + aura (PlayerBridgeService.activateFly, gateado por hito; ver abajo) |
...
Estado en la escena (`jumpsUsed`/`dashing`/`slamming`/`flying`/`arrows`/`usedSecondChance`/`invulnUntil`…), reseteado en `create()`.

**Boost (botón junto al de impulso, hito comprable `boost` 350★).** Botón `app-fly-button` (copia de `sprint-button`, estilo ámbar, `left: 68px` = a la derecha del sprint), gateado por `runMode$` **y** `runMilestones$.includes('boost')` (layout). `activateFly()` también comprueba `hasRunMilestone('boost')`. Lógica de tiempo/cooldown en `PlayerBridgeService` como el sprint: `activateFly()` + `flyActive`/`flyOnCooldown`/`flyCooldownRatio`/`flyCooldownSeconds` (`FLY_DURATION_MS` 10 s, `FLY_COOLDOWN_MS` 30 s; `flyStart` se resetea en `setRunMode`). La escena lee `flyActive` cada frame en `update()` → `startFly()`/`endFly()`. Durante el boost (`this.flying`): X al **×5** (`FLY_SPEED_MULT`, en el `setVelocityX`), **invulnerable** (`damagePlayer` early-return) y **arrasa al contacto** (ramas `this.flying` en updateRats/updateFenixes/updateFireballs, junto a las del dash), con tinte dorado + aura. **NO vuela**: corre y salta con normalidad (gravedad intacta; el vuelo/control de altura anterior se eliminó — nombres `startFly`/`endFly`/`flying` heredados). Aura = doble "ki" estilo Dragon Ball: (1) emisor de partículas doradas ADD (`startFlyAura`/`stopFlyAura`, textura `AURA_KEY` por código) y (2) **aura sprite** `assets/sprites/world_mode/aura/aura_gold.png` (4 frames 100×100 con transparencia; posición `AURA_X_OFFSET`/`AURA_Y_OFFSET`, tamaño `AURA_SPRITE_SCALE`). El PNG se EXTRAJO offline de la fila dorada de `assets/sprites/test/aura.jpg` (JPG fondo blanco 400×4342, ~35 filas de paso ~107px H y ~92px W, auras NO en rejilla): con `jimp` se recortó cada aura por su bbox y se centró en su frame, quitando el blanco a alpha por `min(RGB)` (ver [[aura-sheet-bands]]). `buildAuraSheet` solo registra la anim `AURA_ANIM`. El sprite (`auraSprite`, depth 4 tras el jugador) se crea en `startFly`, se reposiciona en `update()` y se destruye en `endFly`; todo limpiado también en el reset de `create()`. Para OTRO color: re-extraer otra fila a un PNG nuevo (romper caché del navegador con `?v=`). `playerDie` llama `endFly` para limpiar el aura.
...
- **Carteles** (`RUN_UNLOCK_POINTS`, ahora solo POSICIONES: 1-1@100m … 1-8@3600m): `createInterestSigns` los planta; `checkUnlockPoints` al cruzar uno de un mapa YA COMPRADO muestra `showMapEntranceHint` (icono de teletransporte, sin pausar); si no está comprado, decorativo. `entryDistanceFor` sigue usándolos para arrancar en el cartel del mapa de origen. NO desbloquean nada. (ELIMINADO 2026-07: el viejo modal de entrada `promptMapEntrance`/`mapEntrancePrompt$`/`MapEntranceModalComponent`/`dismissMapEntrance` y el desbloqueo AFK por metros `unlockedFlags`/`unlockedMapNames` — eran código muerto sin llamador.)
