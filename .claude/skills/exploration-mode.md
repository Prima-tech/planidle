...
**Vuelo / boost (botón junto al de impulso, SIN hito — disponible siempre en modo mundo).** Botón `app-fly-button` (copia de `sprint-button`, estilo ámbar, `left: 68px` = a la derecha del sprint), gateado solo por `runMode$`. Lógica de tiempo/cooldown en `PlayerBridgeService` como el sprint: `activateFly()` + `flyActive`/`flyOnCooldown`/`flyCooldownRatio`/`flyCooldownSeconds` (`FLY_DURATION_MS` 10 s, `FLY_COOLDOWN_MS` 30 s; `flyStart` se resetea en `setRunMode`). La escena lee `flyActive` cada frame en `update()` → `startFly()`/`endFly()`. Durante el vuelo (`this.flying`): gravedad off, X al **DOBLE** (`FLY_SPEED_MULT` 2, se multiplica en el `setVelocityX`), control de altura en `updateFlight()` (mantener salto = subir `FLY_VERTICAL_SPEED`, soltar = bajar ×0.7, clamp entre `FLY_CEILING_Y` y el suelo), **invulnerable** (`damagePlayer` early-return) y **arrasa al contacto** (ramas `this.flying` en updateRats/updateFenixes/updateFireballs, junto a las del dash). Pose "mirando al suelo" = `setAngle(90)` (pivota en los pies, sin tocar origin ni el body) + tinte dorado, restaurado en `endFly`. Guards `this.flying` en `pressJump`/`startDash`/`startSlam`; `playerDie` llama `endFly` antes de la anim de muerte.
