...
export class MobileHUDScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // ── Attack button ─────────────────────────────────────────────────────────
    const ax = W - 90, ay = H - 115;

    const atkBtn  = this.add.circle(ax, ay, BTN_R, 0xaa1111, 0.72) as Phaser.GameObjects.Arc;
    const atkRing = this.add.circle(ax, ay, BTN_R, 0x000000, 0)    as Phaser.GameObjects.Arc;
    atkRing.setStrokeStyle(2.5, 0xff5555, 0.55);
    const atkLabel = this.add.text(ax, ay, '⚔', { fontSize: '28px', color: '#ffffff' })
    ...
    this.input.on('pointerdown', (p: Phaser.Input.Pointer) => {
      ...
      } else if (p.x >= W * 0.5 && atkId === null) {
        atkId              = p.id;
        input.isAttackHeld = true;
        atkBtn.setFillStyle(0xdd2222, 0.92);
        this.tweens.killTweensOf(atkBtn);
        this.tweens.killTweensOf(atkLabel);
        this.tweens.add({
          targets: [atkBtn, atkRing, atkLabel],
    ...
    this.input.on('pointerup', (p: Phaser.Input.Pointer) => {
      ...
      if (p.id === atkId) {
        ...
        atkBtn.setFillStyle(0xaa1111, 0.72);
        this.tweens.killTweensOf(atkBtn);
        this.tweens.killTweensOf(atkLabel);
        this.tweens.add({
          targets: [atkBtn, atkRing, atkLabel],
